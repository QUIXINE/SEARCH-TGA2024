...
public class ConveyorBelt : MonoBehaviour
{
    ...
    private void OnTriggerStay(Collider col)
    {
            ...
            && woods != null)
        {
            ...
            /*if (IsMoving && woods[0].transform.parent)
            {
                for (int i = 0; i < woods.Count; i++)
                {
                    if (woods[i] != null)
                    {
                        if (woods[i].transform.parent.gameObject.GetComponent<MovableObjectCheckMoving>().IsMoving)
                        {
                            woods[i].transform.parent.position += Vector3.right * _woodMoveSpeed * Time.deltaTime;
                        }
                    }
                    /*woods[i].GetComponent<Rigidbody>().MovePosition(woods[i].transform.position +
                                                                    (woods[i].transform.right * _moveSpeed * Time.deltaTime));#1#
                }
            }*/
        }
            ...
            col.gameObject.CompareTag(TagManager.MovableItem))
        {
            ...
            for (int i = _woodAmount; i <= woods.Count; i++)
            {
                if(i == 0)
                {
                    ...
                    {
                        print("Assign 0 parent");
                        woods.Add(col.transform.parent.gameObject);
                        woods[i].gameObject.GetComponent<MovableObjectCheckMoving>().IsMoving = true;
                        _woodAmount++;
                    }*/
                    else if (woods.Count == 0 && !col.transform.parent)
                    {
    ...
    {
        if (col.gameObject.TryGetComponent<MovableObjectCheckMoving>(out var movableObj1) &&
            col.gameObject.CompareTag(TagManager.MovableItem))
        {
            movableObj1.IsMoving = false;
            _woodAmount--;
            woods.Remove(col.gameObject);
        }
        /*else if (col.gameObject.TryGetComponent<MovableObjectCheckMoving>(out var movableObj2) &&
                 col.gameObject.CompareTag(TagManager.MovableItem) && col.transform.parent)
        {
            movableObj2.IsMoving = false;
            _woodAmount--;
            woods.Remove(col.transform.parent.gameObject);
        }*/
